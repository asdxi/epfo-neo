...
export const ACCOUNT_STORAGE_KEY = 'epfo-neo-account-v7'
...
export function loadPersistedAccount(storage: Pick<Storage, 'getItem'>): AccountState {
  ...
  try {
    ...
    if (parsed.version !== 7 || !parsed.member || !parsed.ledger || !Array.isArray(parsed.requests) || !Array.isArray(parsed.member.nominees)) return createInitialAccount()
