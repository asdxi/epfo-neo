...
export const ACCOUNT_STORAGE_KEY = 'epfo-neo-account-v2'
...
export function loadPersistedAccount(storage: Pick<Storage, 'getItem'>): AccountState {
  ...
  try {
    ...
    if (parsed.version !== 2 || !parsed.member || !parsed.ledger || !Array.isArray(parsed.requests)) return createInitialAccount()
