...
export const ACCOUNT_STORAGE_KEY = 'epfo-neo-account-v4'
...
export function loadPersistedAccount(storage: Pick<Storage, 'getItem'>): AccountState {
  ...
  try {
    ...
    if (parsed.version !== 4 || !parsed.member || !parsed.ledger || !Array.isArray(parsed.requests) || !Array.isArray(parsed.member.nominees)) return createInitialAccount()
