...
const formatWageMonth = (value: string) => new Intl.DateTimeFormat('en-IN', { month: 'long', year: 'numeric' }).format(new Date(`${value}-01T00:00:00`))
...
function FlowProgress({ service, step }: { service: ServiceId; step: FlowStep }) {
  ...
  return <ol className="service-progress" aria-label={`${serviceNames[service]} progress`}>
    {labels.map((label, index) => <li key={label} className={index < active ? 'is-complete' : index === active ? 'is-current' : ''} aria-current={index === active ? 'step' : undefined}><span>{index + 1}</span>{label}</li>)}
  </ol>
}
...
function TransferFlow({ account, step, setStep, onSubmitTransfer, onViewRequests }: FlowProps) {
  ...
  if (exception?.state === 'in-progress' && step !== 'outcome') return <div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Transfer Is Already in Progress</p><p className="ux4g-alert-message">The balance remains under the previous Member ID until processing completes. Review its status and next expected step in Requests.</p><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => onViewRequests?.(exception.relatedRequestId)}>Track Transfer</button></div></div>
...
function ClaimFlow({ account, step, setStep, onSubmitClaim, onViewRequests }: FlowProps) {
  ...
  const amountError = !Number.isFinite(numericAmount) || numericAmount <= 0 ? 'Enter an amount greater than zero.' : numericAmount > 25_000 ? 'For this synthetic scenario, enter no more than ₹25,000.' : ''
  ...
  return <article className="service-flow">{step === 'explain' && <div className="service-flow-body"><h2>Choose a Claim Intent</h2><p>Eligibility depends on the claim type and member circumstances. This account view can only pre-check the records it holds; EPFO review determines the outcome.</p><FlowActions step={step} setStep={setStep} /></div>}
    {step === 'details' && <form className="service-flow-body" onSubmit={(event) => event.preventDefault()}><Field id="claim-intent" label="Claim intent"><select id="claim-intent" className="service-select" value={intent} onChange={(event) => setIntent(event.target.value)}><option value="advance">Request a PF Advance</option><option value="final">Final EPF Settlement</option><option value="pension">Pension-related Claim</option></select></Field>{ineligible ? <div className="ux4g-alert ux4g-alert-warning"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Cannot Be Submitted from This Account State</p><p className="ux4g-alert-message">The current employment record is active. This prototype does not have enough verified circumstances to establish eligibility for this claim intent. Choose PF Advance to continue with the synthetic scenario.</p></div></div> : <><div className="service-readiness-line"><span>Verified bank {bank?.maskedValue}</span><StatusTag state={bank?.state ?? 'unverified'} /></div><Field id="claim-amount" label="Amount requested" error={amountError} help="Synthetic amount available for this scenario: up to ₹25,000."><input id="claim-amount" className={`ux4g-input ux4g-input-md ${amountError ? 'ux4g-input-error' : ''}`} inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value.replace(/\D/g, ''))} aria-invalid={Boolean(amountError)} aria-describedby={`claim-amount-${amountError ? 'error' : 'help'}`} /></Field></>}<FlowActions step={step} setStep={setStep} disabled={ineligible || Boolean(amountError)} /></form>}
    {step === 'review' && <div className="service-flow-body"><h2>Review Claim</h2><dl className="service-review-list"><div><dt>Claim Type</dt><dd>PF Advance</dd></div><div><dt>Amount Requested</dt><dd>{formatMoney(numericAmount)}</dd></div><div><dt>Payment Account</dt><dd>{bank?.maskedValue ?? 'Unavailable'}<small>{bank?.state === 'verified' ? 'Verified' : 'Needs verification'}</small></dd></div></dl><label className="ux4g-checkbox ux4g-checkbox-md"><input className="ux4g-checkbox-input" type="checkbox" checked={declaration} onChange={(event) => setDeclaration(event.target.checked)} /><span className="ux4g-checkbox-control"><span className="ux4g-checkmark" /></span><span className="ux4g-checkbox-content">I confirm the information shown is correct for this synthetic request.</span></label><FlowActions step={step} setStep={setStep} disabled={!declaration} onConfirm={submit} confirmLabel="Verify and Submit Claim" /></div>}
...
function GrievanceFlow({ account, step, setStep, onSubmitGrievance, onViewRequests, initialEmploymentId, initialContributionId }: FlowProps) {
  ...
  const [support, setSupport] = useState('June 2026 contribution record from this synthetic passbook')
  ...
  return <article className="service-flow service-flow--hero">{step === 'explain' && <div className="service-flow-body"><h2>Get a Trackable Response</h2><p>Use a grievance when a contribution or member service issue needs review. Select the exact employment or transaction so the complaint reaches the right record.</p><div className="ux4g-alert ux4g-alert-warning"><div className="ux4g-alert-content"><p className="ux4g-alert-title">The Record Does Not Explain the Cause</p><p className="ux4g-alert-message">{relatedContribution ? `${relatedContribution.explanation} Wage Month: ${formatWageMonth(relatedContribution.wageMonth)}.` : 'The selected record needs review.'} This account does not infer why.</p></div></div><FlowActions step={step} setStep={setStep} /></div>}
