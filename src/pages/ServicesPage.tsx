...
function TransferFlow({ account, step, setStep, onSubmitTransfer, onViewRequests }: FlowProps) {
  ...
  return <article className="service-flow service-flow--hero">
    {step === 'explain' && <div className="service-flow-body"><div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-message">Eligible PF balances are usually transferred automatically. Use this service when a previous balance has not been transferred.</p></div></div><h2>Select a Previous Employment</h2><Field id="transfer-source" label="Source Employment"><select id="transfer-source" className="service-select" value={sourceMemberId} onChange={(event) => { setSourceMemberId(event.target.value); setAccepted(false) }}>{selectable.map((item) => <option key={item.employment.memberId} value={item.employment.memberId}>{item.employment.employer} · {item.employment.memberId}</option>)}</select></Field>{candidate.result.state === 'existing-manual-request' ? <div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Transfer Already in Progress</p><p className="ux4g-alert-message">A transfer request for this employment is already being processed.</p><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => onViewRequests?.(candidate.result.state === 'existing-manual-request' ? candidate.result.requestId : undefined)}>Track Request</button></div></div> : <><p>{formatMoney(amount)} is recorded under this Member ID. The destination is {destination.employer}, {destination.memberId}.</p><div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">What Happens Next</p><p className="ux4g-alert-message">The previous employment record and Member IDs will be used to process the transfer. The money remains under the source Member ID until completion.</p></div></div><FlowActions step={step} setStep={setStep} /></>} {candidates.some((item) => item.result.state === 'automatic-completed') && <p className="service-transfer-history">Earlier eligible balances shown in your record were transferred automatically.</p>}</div>}
