...
import { formatWageMonth, totalEpfBalance } from '../domain/calculations'
...
function ClaimFlow({ account, step, setStep, onSubmitClaim, onViewRequests }: FlowProps) {
  ...
  const maximumAmount = Math.max(0, Math.floor(intent === 'final' ? (fullSettlementEligible ? balanceForClaim : latestEmployment?.exitedOn ? balanceForClaim * 0.75 : 0) : balanceForClaim * 0.75))
  const reasonOptions = intent === 'final' ? fullWithdrawalReasons : partialWithdrawalReasons
  const unverifiedKyc = account.kyc.filter((record) => record.state !== 'verified').map((record) => record.type === 'aadhaar' ? 'Aadhaar' : record.type === 'pan' ? 'PAN' : 'Bank')
  const blockedFullWithdrawal = intent === 'final' && !fullSettlementEligible
  const numericAmount = Number(amount)
  const amountError = blockedFullWithdrawal || maximumAmount === 0 ? '' : !Number.isFinite(numericAmount) || numericAmount <= 0 ? 'Enter an amount greater than zero.' : numericAmount > maximumAmount ? `Enter no more than ${formatMoney(maximumAmount)}.` : ''
  ...
  return <article className="service-flow">{step === 'explain' && <div className="service-flow-body"><h2>Choose a Withdrawal Type</h2><p>Start with the kind of withdrawal you need. The next step will show only the reasons that apply.</p><p>PF Advance is a partial withdrawal. Full PF Withdrawal settles your overall PF balance when you meet the unemployment or special-condition rules.</p><FlowActions step={step} setStep={setStep} /></div>}
    {step === 'details' && <form className="service-flow-body" onSubmit={(event) => event.preventDefault()}>{hasIncompleteKyc && <div className="ux4g-alert ux4g-alert-warning"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Demo KYC Allowance</p><p className="ux4g-alert-message">For this demo, you can continue filing this request. In the live EPFO portal, Aadhaar, PAN and bank KYC must be completed for online claims.</p></div></div>}<Field id="claim-intent" label="Withdrawal Type"><select id="claim-intent" className="service-select" value={intent} onChange={(event) => { const next = event.target.value as WithdrawalKind; setIntent(next); setReason(next === 'final' ? fullWithdrawalReasons[0] : partialWithdrawalReasons[0]); setAmount('') }}><option value="advance">PF Advance · Partial Withdrawal</option><option value="final">Full PF Withdrawal · Final Settlement</option></select></Field><Field id="claim-reason" label={intent === 'advance' ? 'Reason for PF Advance' : 'Reason for Full PF Withdrawal'}><select id="claim-reason" className="service-select" value={reason} onChange={(event) => setReason(event.target.value)}>{reasonOptions.map((option) => <option key={option}>{option}</option>)}</select></Field>{intent === 'final' && <>{blockedFullWithdrawal ? <div className="ux4g-alert ux4g-alert-warning"><div className="ux4g-alert-content"><p className="ux4g-alert-title">You’re Currently Employed</p><p className="ux4g-alert-message">Mark your exit from {latestEmployment?.employer ?? 'your latest employer'} before applying for a full withdrawal.{pendingTransferToLatestEmployment ? ' Your pending PF transfer must also complete before those funds can be withdrawn.' : ''}</p></div></div> : <div className="service-readiness-line"><span>Latest employment ended {formatDate(latestEmployment?.exitedOn ?? '')} · {unemploymentMonths} month{unemploymentMonths === 1 ? '' : 's'} unemployed</span></div>}</>}<div className="service-readiness-line service-kyc-summary"><strong>KYC status</strong><span>{unverifiedKyc.length ? `Not verified: ${unverifiedKyc.join(', ')}` : 'All KYC details verified'}</span></div>{intent === 'advance' && <div className="service-readiness-line"><span>Up to 75% of your available EPF balance</span></div>}<Field id="claim-amount" label="Amount requested" error={amountError} help={intent === 'advance' ? `Up to 75% of the available EPF balance: ${formatMoney(maximumAmount)}.` : 'Enter an amount after your exit is recorded and any pending transfer is complete.'}><input id="claim-amount" className={`ux4g-input ux4g-input-md ${amountError ? 'ux4g-input-error' : ''}`} inputMode="numeric" value={amount} disabled={blockedFullWithdrawal || maximumAmount === 0} onChange={(event) => setAmount(event.target.value.replace(/\D/g, ''))} aria-invalid={Boolean(amountError)} aria-describedby={`claim-amount-${amountError ? 'error' : 'help'}`} /></Field><FlowActions step={step} setStep={setStep} disabled={blockedFullWithdrawal || Boolean(amountError)} /></form>}
    {step === 'review' && <div className="service-flow-body"><h2>Review Withdrawal</h2><dl className="service-review-list"><div><dt>Withdrawal Type</dt><dd>{intent === 'advance' ? 'PF Advance · Partial Withdrawal' : 'Full PF Withdrawal · Final Settlement'}</dd></div><div><dt>Reason</dt><dd>{reason}</dd></div>{intent === 'final' && <div><dt>Eligibility</dt><dd>{fullSettlementEligible ? '2+ months completely unemployed · 100% available' : 'Not eligible for full settlement'}</dd></div>}<div><dt>Amount Requested</dt><dd>{formatMoney(numericAmount)}</dd></div><div><dt>Payment Account</dt><dd>{bank?.maskedValue ?? 'Unavailable'}<small>{bank?.state === 'verified' ? 'Verified' : 'Needs verification'}</small></dd></div></dl><label className="ux4g-checkbox ux4g-checkbox-md"><input className="ux4g-checkbox-input" type="checkbox" checked={declaration} onChange={(event) => setDeclaration(event.target.checked)} /><span className="ux4g-checkbox-control"><span className="ux4g-checkmark" /></span><span className="ux4g-checkbox-content">I confirm the information shown is correct for this request.</span></label><FlowActions step={step} setStep={setStep} disabled={!declaration} onConfirm={submit} confirmLabel="Verify and Submit Withdrawal" /></div>}
