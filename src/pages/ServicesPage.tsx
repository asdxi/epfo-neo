...
function Outcome({ request, title, message, onViewRequests, alertClass = 'ux4g-alert-success', className = '' }: { request?: MemberRequest; title: string; message: string; onViewRequests?: (requestId?: string) => void; alertClass?: string; className?: string }) {
  return <div className={`ux4g-alert ${alertClass} service-outcome ${className}`.trim()} role="status">
    <div className="ux4g-alert-content"><h3 className="ux4g-alert-title">{title}</h3><p className="ux4g-alert-message">{message}</p>
      {request && <dl className="service-outcome-reference"><div><dt>Neo case ID</dt><dd>{request.reference}</dd></div><div><dt>Attempted On</dt><dd>{formatDate(request.submittedOn)}</dd></div></dl>}
...
function TransferFlow({ account, step, setStep, onSubmitTransfer, onViewRequests }: FlowProps) {
  ...
  return <article className="service-flow service-flow--hero">
    ...
    {step === 'outcome' && <div className="service-flow-body"><Outcome request={request} title="Transfer attempt saved" message={`${formatMoney(exception.amount)} remains under ${source.employer}. Neo has not confirmed a portal receipt or EPFO acknowledgement; track this existing attempt before taking another action.`} onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>}
  </article>
}
...
function ClaimFlow({ account, step, setStep, onSubmitClaim, onViewRequests }: FlowProps) {
  ...
  return <article className="service-flow">{step === 'explain' && <div className="service-flow-body"><h2>Choose a Claim Intent</h2><p>Eligibility depends on the claim type and member circumstances. This account view can only pre-check the records it holds; EPFO review determines the outcome.</p><FlowActions step={step} setStep={setStep} /></div>}
    ...
    {step === 'outcome' && <div className="service-flow-body"><Outcome request={request} title="Claim attempt saved" message={`Neo saved the ${formatMoney(numericAmount)} claim attempt. A claims-portal receipt is not confirmed, and payment is not guaranteed.`} onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>}
  </article>
}
...
function CorrectionFlow({ account, step, setStep, onSubmitCorrection, onViewRequests, initialEmploymentId }: FlowProps) {
  ...
  return <article className="service-flow">{step === 'explain' && <div className="service-flow-body"><h2>Correct a Recorded Employment Detail</h2><p>Select the employer and field, then provide the value you believe is correct. The employer is expected to verify it before EPFO review.</p><FlowActions step={step} setStep={setStep} /></div>}
    ...
    {step === 'outcome' && <div className="service-flow-body"><Outcome request={request} title="Correction attempt saved" message="Neo has saved the attempt. Track it in Requests to confirm receipt before employer review." onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>}
  </article>
}
...
function GrievanceFlow({ account, step, setStep, onSubmitGrievance, onViewRequests, initialEmploymentId, initialContributionId }: FlowProps) {
  ...
  return <article className="service-flow service-flow--hero">{step === 'explain' && <div className="service-flow-body"><h2>Get a Trackable Response</h2><p>Use a grievance when a contribution or member service issue needs review. In the next step, choose the employment or transaction you want EPFO to review.</p><FlowActions step={step} setStep={setStep} /></div>}
    {step === 'details' && <form className="service-flow-body" onSubmit={(event: FormEvent) => event.preventDefault()}>{contextualResolution ? <><div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Neo prepared this review</p><p className="ux4g-alert-message">Employer, Wage Month, recorded amounts and references are carried from PF Record Review. Confirm the prepared evidence instead of entering it again.</p></div></div><dl className="service-review-list"><div><dt>Employer</dt><dd>{employment?.employer}</dd></div><div><dt>Contribution</dt><dd>{contributionId}</dd></div><div><dt>Issue category</dt><dd>{contextualResolution.categoryLabel}</dd></div><div><dt>References</dt><dd>{contextualResolution.references.join(', ')}</dd></div></dl></> : <div className="service-form-grid"><Field id="grievance-employer" label="Affected employment"><select id="grievance-employer" className="service-select" value={employmentId} onChange={(event) => { setEmploymentId(event.target.value); setContributionId('') }}>{account.employments.map((item) => <option key={item.id} value={item.id}>{item.employer}</option>)}</select></Field><Field id="grievance-transaction" label="Affected contribution (optional)"><select id="grievance-transaction" className="service-select" value={contributionId} onChange={(event) => setContributionId(event.target.value)}><option value="">Employment record generally</option>{contributions.map((item) => <option key={item.id} value={item.id}>{item.wageMonth} | {item.status.replaceAll('-', ' ')}</option>)}</select></Field></div>}<Field id="grievance-category" label="Issue category"><select id="grievance-category" className="service-select" value={category} onChange={(event) => setCategory(event.target.value)} disabled={Boolean(contextualResolution)}>{contextualResolution ? contextualResolution.validCategories.map((item) => <option key={item}>{contributionDiscrepancyLabel(item)}</option>) : <><option>Contribution Amount Not Fully Recorded</option><option>Contribution Not Recorded</option><option>Transfer Delay</option><option>Claim Status</option><option>Employment Record Issue</option></>}</select></Field><Field id="grievance-description" label="Prepared issue description" error={description && invalid ? 'Add at least 20 characters so the issue can be understood.' : undefined} help="Neo prepared this from the selected record. You may add factual context without including real personal data."><textarea id="grievance-description" className="service-textarea" rows={5} maxLength={1000} value={description} onChange={(event) => setDescription(event.target.value)} aria-describedby={`grievance-description-${description && invalid ? 'error' : 'help'}`} /></Field><Field id="grievance-support" label="Evidence already held by Neo" help="These synthetic references are attached to this prototype request; no external file is uploaded."><input id="grievance-support" className="ux4g-input ux4g-input-md" value={support} readOnly={Boolean(contextualResolution)} onChange={(event) => setSupport(event.target.value)} /></Field><FlowActions step={step} setStep={setStep} disabled={invalid} /></form>}
    ...
    {step === 'outcome' && <div className="service-flow-body"><Outcome request={request} title="Grievance attempt saved" message="Neo has prepared and saved the attempt. A grievance-portal receipt and EPFO acknowledgement are not confirmed yet." onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>}
  </article>
}
...
function ExitFlow({ account, step, setStep, onSubmitExit, onViewRequests }: FlowProps) {
  ...
  if (step === 'outcome') return <div className="exit-submission-panel"><Outcome request={request} title="Exit attempt saved" message="Neo saved the exit attempt. Track it to confirm the member-portal receipt before making a withdrawal claim." onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>
