...
function TransferFlow({ account, step, setStep, onSubmitTransfer, onViewRequests }: FlowProps) {
  ...
  const automaticCompleted = account.ledger.transfers.find((item) => item.state === 'completed' && item.fromMemberId === source?.memberId && item.toMemberId === destination?.memberId)
  const preflight = exception?.state === 'in-progress' && exception.relatedRequestId ? 'existing-request' : automaticCompleted ? 'automatic-completed' : exception?.amount ? 'manual-required' : 'none'
  if (exception?.state === 'in-progress' && step !== 'outcome') return <div><p className="service-flow-intro">Eligible PF balances are usually transferred automatically. Use this service when a previous balance remains.</p><div className="ux4g-alert ux4g-alert-info service-transfer-progress-alert"><div className="ux4g-alert-content"><div className="service-transfer-progress-copy"><p className="ux4g-alert-title">Transfer Is Already in Progress</p><p className="ux4g-alert-message">The balance remains under the previous Member ID until processing completes. Review its status and next expected step in Requests.</p></div><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => onViewRequests?.(exception.relatedRequestId)}>Track Transfer</button></div></div></div>
  if (automaticCompleted && preflight === 'automatic-completed' && step !== 'outcome') return <div className="ux4g-alert ux4g-alert-success"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Automatic Transfer Completed</p><p className="ux4g-alert-message">The eligible previous balance was transferred automatically. No manual action is needed.</p><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => onViewRequests?.(automaticCompleted.relatedRequestId)}>View Transfer</button></div></div>
  if (preflight === 'none' || !source || !destination || !exception?.amount) return <div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">No Transfer Action Is Needed</p><p className="ux4g-alert-message">There is no previous PF balance requiring transfer.</p></div></div>
  return <article className="service-flow service-flow--hero">
    ...
    {step === 'outcome' && <div className="service-flow-body"><Outcome request={request} title="Request Filed" message={`${formatMoney(exception.amount)} remains under ${source.employer} until the transfer completes. Track this request in Requests.`} onViewRequests={onViewRequests} alertClass="ux4g-alert-info" /></div>}
  </article>
}
