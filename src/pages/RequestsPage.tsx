...
export function RequestsPage({ account, initialRequestId, status = 'ready', onRetry, onCitizenAction }: RequestsPageProps) {
  ...
  if (status === 'loading') return <section className="service-page" aria-labelledby="requests-title"><header className="service-page-heading"><p className="service-eyebrow">Operational history</p><h1 id="requests-title">Requests</h1></header><RequestSkeleton /></section>
  if (status === 'error') return <section className="service-page" aria-labelledby="requests-title"><header className="service-page-heading"><p className="service-eyebrow">Operational history</p><h1 id="requests-title">Requests</h1></header><div className="ux4g-alert ux4g-alert-error" role="alert"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Requests Could Not Be Loaded</p><p className="ux4g-alert-message">Your account data is safe. Try loading this page again.</p>{onRetry && <button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={onRetry}>Try Again</button>}</div></div></section>

  return <section className="service-page" aria-labelledby="requests-title">
    <header className="service-page-heading"><p className="service-eyebrow">Operational history</p><h1 id="requests-title">Requests</h1><p>Track services that take time, see the next expected step, and respond when action is required.</p></header>
    <div className="ux4g-tab-list ux4g-tab-underline ux4g-tab-md request-view-tabs" role="tablist" aria-label="Request status">
      {(['open', 'completed'] as const).map((item) => <button key={item} className={`ux4g-tab-item ${view === item ? 'active' : ''}`} type="button" role="tab" aria-selected={view === item} onClick={() => { setView(item); setSelectedId('') }}>{titleCase(item)}<span className="request-tab-count">{account.requests.filter((request) => item === 'completed' ? request.state === 'completed' : request.state !== 'completed').length}</span></button>)}
    </div>
    <div className="request-filters" role="group" aria-label="Filter by request type">{(['all', 'claim', 'transfer', 'correction', 'grievance'] as RequestFilter[]).map((item) => <button key={item} className={`ux4g-filter-chip-md ${filter === item ? 'active' : ''}`} type="button" aria-pressed={filter === item} onClick={() => { setFilter(item); setSelectedId('') }}>{item === 'all' ? 'All' : item === 'claim' ? 'Claims' : item === 'transfer' ? 'Transfers' : item === 'correction' ? 'Corrections' : 'Grievances'}</button>)}</div>
    {requests.length === 0 ? <div className="request-empty"><h2>No {filter === 'all' ? '' : `${titleCase(filter)} `}{view === 'open' ? 'Open' : 'Completed'} Requests</h2><p>{view === 'open' ? 'There is nothing waiting for review or action in this view.' : 'Completed requests will appear here with their final status and dates.'}</p></div> : <div className="request-layout">
...
function RequestDetail({ request, onCitizenAction }: { request: MemberRequest; onCitizenAction?: (request: MemberRequest) => void }) {
  return <article className="request-detail" aria-live="polite" aria-labelledby="request-detail-title">
    <header className="request-detail-header"><div><p className="service-eyebrow">{request.service}</p><h2 id="request-detail-title">{request.title}</h2></div><span className={`ux4g-tag-filled-${statusTone[request.state]} ux4g-tag-s`}>{titleCase(request.state)}</span></header>
