...
describe('Services page navigation', () => {
  ...
  it('uses browser history to return from a service action to the catalogue', async () => {
    ...
    expect(container.textContent).not.toContain('Manage Nomination in Account')
    ...
    expect(container.querySelector('.service-danger-zone')?.textContent).toContain('Mark Exit')
    expect(container.querySelector('.service-danger-zone')?.textContent).not.toContain('Exit from EPFO Scheme')
    expect(container.querySelector('.service-danger-zone button')?.textContent).toBe('Mark Exit')
