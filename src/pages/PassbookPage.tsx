...
export function PassbookPage(props: PassbookPageProps) {
  ...
  return <section className="financial-page passbook-page" aria-labelledby="passbook-title">
    <header className="financial-heading"><h1 id="passbook-title">Passbook</h1><p>See your balance, recent contributions, employer totals and ledger.</p></header>
...
function Overview({ account, contributions, highlightedContributionId, rail = false, onRaiseContributionGrievance }: { account: AccountState; contributions: ContributionRecord[]; highlightedContributionId?: string; rail?: boolean; onRaiseContributionGrievance: (contribution: ContributionRecord) => void }) {
  ...
  const rows = [
    ['Employee Contributions', value.employeeContributions],
  ...
  const serviceRemaining = Math.max(0, 120 - serviceMonths)
  ...
  const remainingLabel = `${Math.floor(serviceRemaining / 12)} years ${serviceRemaining % 12} months`
  return <section className="passbook-panel" id="passbook-panel-overview" role={rail ? 'region' : 'tabpanel'} aria-label={rail ? 'Overview' : undefined} tabIndex={-1}>
    <section className="financial-section overview-recent" aria-labelledby="recent-contributions-title"><div className="financial-section-heading"><div><h2 id="recent-contributions-title">Recent Contributions</h2><p>EPF recorded in the latest six months.</p></div></div>{contributions.length === 0 ? <Empty title="No Contributions Recorded">No contribution records are available for the last six months.</Empty> : <ol className="contribution-summary-grid">{contributions.map((record) => { const employer = account.employments.find((item) => item.id === record.employmentId); const highlighted = record.id === highlightedContributionId; return <li key={record.id} className={highlighted ? 'highlighted' : ''}><time dateTime={record.wageMonth}>{formatWageMonth(record.wageMonth)}</time><span className="contribution-employer">{employer?.employer ?? 'Employer Unavailable'}</span><div className="contribution-amount"><span>Employee EPF</span><strong>{record.employeeEpf === null ? <span className="record-missing-value">Not Recorded</span> : formatMoney(record.employeeEpf)}</strong></div><div className="contribution-amount"><span>Employer EPF</span><strong>{record.employerEpf === null ? <span className="record-missing-value">Not Recorded</span> : formatMoney(record.employerEpf)}</strong></div><small>Recorded {formatDate(record.recordedOn)}</small>{highlighted && record.employerEpf === null && <button className="ux4g-btn ux4g-btn-primary ux4g-btn-lg contribution-review-action" type="button" onClick={() => onRaiseContributionGrievance(record)}>Request Review</button>}</li> })}</ol>}</section>
    <section className="financial-panel overview-calculation" id="balance-calculation-section" aria-labelledby="balance-composition-title" tabIndex={-1}><h2 id="balance-composition-title">How This Is Calculated</h2><dl className="balance-composition">{rows.map(([label, amount, help]) => <div key={label}><dt><span>{label}</span>{help && <InfoTooltip id={`calculation-${label.toLowerCase().replaceAll(' ', '-')}`} label={`Explain ${label}`}>{help}</InfoTooltip>}</dt><dd>{formatMoney(amount)}</dd></div>)}<div className="balance-composition-total"><dt>Current EPF Balance</dt><dd>{formatMoney(value.closingBalance)}</dd></div></dl></section>
    <aside className="ux4g-alert ux4g-alert-info overview-eps"><div className="ux4g-alert-content"><p className="ux4g-alert-title">EPS Is Separate</p><p className="ux4g-alert-message">{formatMoney(totalEpsContributions(account))} is recorded toward pension service, not included in your EPF balance.</p><dl className="eps-eligibility"><div><dt>Pension Service</dt><dd>{serviceLabel}</dd></div><div><dt>To Reach 10 Years</dt><dd>{serviceRemaining > 0 ? `${remainingLabel} more` : 'Completed'}</dd></div><div><dt>Normal Pension Age</dt><dd>58 · {formatDate(pensionDate)}</dd></div></dl><p className="ux4g-alert-message">With 10 years of eligible service, normal pension starts at 58. Reduced pension may start from 50 after leaving employment.</p></div></aside>
  </section>
}
...
function EmployerDetail({ summary, highlighted, onStartTransfer }: { summary: ReturnType<typeof employerSummaries>[number]; highlighted: boolean; onStartTransfer: (employmentId: string) => void }) {
  ...
  const metric = (label: string, amount: number, sign: '+' | '-') => <div><dt>{label}</dt><dd>({sign}) {formatMoney(amount)}</dd></div>
  return <article className={`financial-panel employer-detail${highlighted ? ' context-target-highlight' : ''}`} id={`employer-detail-${employment.id}`} tabIndex={-1}><div className="financial-section-heading"><div><h3>{employment.employer}</h3><p>{formatDate(employment.joinedOn)} to {employment.exitedOn ? formatDate(employment.exitedOn) : 'Present'}</p></div>{summary.transferState !== 'not-applicable' && <span className="ux4g-tag ux4g-tag-filled-neutral ux4g-tag-s employer-transfer-tag">{transferStateLabel(summary.transferState)}</span>}</div><section className="employer-accounts" aria-labelledby={`employer-accounts-${employment.id}`}><h4 id={`employer-accounts-${employment.id}`}>PF Accounts</h4><ul>{accounts.map((account) => <li key={account.accountNumber}><strong>{account.entity}</strong><span>{providerLabel(account.provider)}</span><span className="employer-account-number">{account.accountNumber}</span></li>)}</ul></section>{employment.dataAvailability !== 'complete' && <div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Historical Data {employment.dataAvailability === 'partial' ? 'Is Partial' : 'Is Unavailable'}</p><p className="ux4g-alert-message">{employment.dataAvailabilityNote ?? 'Some historical entries are not available in this account.'}</p></div></div>}<dl className="employer-metrics">{metric('Employee Contributions', summary.employeeContributions, '+')}{metric('Employer EPF Contributions', summary.employerEpfContributions, '+')}{metric('EPS Contributions', summary.epsContributions, '+')}{metric('Interest Credited', summary.officialInterestCredits, '+')}{metric('Transfers In', summary.transfersIn, '+')}{metric('Transfers Out', summary.transfersOut, '-')}{metric('Withdrawals', summary.withdrawals, '-')}<div className="employer-balance-total"><dt>Closing EPF Balance</dt><dd>{formatMoney(summary.closingBalance)}</dd></div></dl>{employment.status === 'balance-remaining' && !transferInProgress && <button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="button" onClick={() => onStartTransfer(employment.id)}>Transfer Previous PF</button>}{transferInProgress && <div className="ux4g-alert ux4g-alert-info"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Transfer Is in Progress</p><p className="ux4g-alert-message">This balance remains here until the submitted transfer is processed.</p></div></div>}</article>
}
  ...
  return <div className="transaction-results">
    <div className="transaction-table-scroll" tabIndex={0} role="region" aria-label="Transaction results table">
      <table className="ux4g-table ux4g-table-m ux4g-table-zebra-rows transaction-table">
        ...
        <thead><tr><th scope="col">Date</th><th scope="col">Transaction</th><th scope="col">Employer</th><th scope="col">Type</th><th scope="col" className="transaction-amount-column">Employee EPF</th><th scope="col" className="transaction-amount-column">Employer EPF</th><th scope="col" className="transaction-amount-column">Total</th></tr></thead>
        <tbody>{pageTransactions.map((transaction) => { const employment = account.employments.find((item) => item.id === transaction.employmentId); const highlighted = transaction.id === highlightedTransactionId; const contribution = transaction.type === 'contribution'; const employeeEpf = typeof transaction.employeeEpf === 'number' ? formatMoney(transaction.employeeEpf) : <span className="record-missing-value">Not Recorded</span>; const employerEpf = typeof transaction.employerEpf === 'number' ? formatMoney(transaction.employerEpf) : <span className="record-missing-value">Not Recorded</span>; return <tr key={transaction.id} id={`transaction-${transaction.id}`} className={highlighted ? 'context-target-highlight' : undefined} tabIndex={highlighted ? -1 : undefined}><td>{formatDate(transaction.date)}</td><td>{transaction.title}</td><td>{employment?.employer ?? 'Employer Unavailable'}</td><td><span className="ux4g-tag ux4g-tag-filled-neutral ux4g-tag-s">{transactionTypeLabel[transaction.type]}</span></td><td className="transaction-amount-column">{contribution ? employeeEpf : 'N/A'}</td><td className="transaction-amount-column">{contribution ? employerEpf : 'N/A'}</td><td className="transaction-amount-column">{transaction.amount === null ? 'Unavailable' : formatMoney(transaction.amount)}</td></tr> })}</tbody>
      </table>
    </div>
