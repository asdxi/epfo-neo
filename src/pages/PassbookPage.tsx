...
function Overview({ account, contributions, highlightedContributionId, rail = false, onRaiseContributionGrievance }: { account: AccountState; contributions: ContributionRecord[]; highlightedContributionId?: string; rail?: boolean; onRaiseContributionGrievance: (contribution: ContributionRecord) => void }) {
  ...
  return <section className="passbook-panel" id="passbook-panel-overview" role={rail ? 'region' : 'tabpanel'} aria-label={rail ? 'Overview' : undefined} tabIndex={-1}>
    ...
    <aside className="ux4g-alert ux4g-alert-info overview-eps"><div className="ux4g-alert-content"><div className="eps-title-row"><p className="ux4g-alert-title">EPS</p><InfoTooltip id="eps-explanation" label="Explain EPS">EPS is separate from EPF and recorded toward pension.</InfoTooltip></div><p className="eps-contribution-total"><span>Recorded Pension Contributions</span><strong>{formatMoney(totalEpsContributions(account))}</strong></p><ul className="eps-rules"><li><strong>Your Service:</strong> {serviceLabel}. {serviceRemaining > 0 ? `${remainingLabel} to reach 10 years.` : 'You have reached 10 years.'}</li><li><strong>Before 10 Years:</strong> After leaving employment, take a withdrawal benefit or keep the service for a future job.</li><li><strong>After 10 Years:</strong> No lump-sum withdrawal. EPS provides a monthly pension.</li><li><strong>Pension Start:</strong> At 58 ({formatDate(pensionDate)}), or at a reduced rate from 50 after leaving employment.</li><li><strong>Payment:</strong> Monthly, not yearly or on demand.</li></ul></div></aside>
  </section>
}
