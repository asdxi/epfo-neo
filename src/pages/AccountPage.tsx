...
function ContactEditor({ account, onUpdateContact }: AccountPageProps) {
  ...
  if (step === 'read') return <div className="contact-readonly"><div><span>Mobile Number</span><strong>{maskMobile(account.member.mobile.value)}</strong><span className="ux4g-tag-filled-success ux4g-tag-s">Verified</span></div><div className="contact-readonly-email"><div><span>Email ID</span><strong>{maskEmail(account.member.email.value)}</strong>{account.member.email.verified && <span className="ux4g-tag-filled-success ux4g-tag-s">Verified</span>}</div><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => { selectChannel('email'); setStep('edit') }}>{account.member.email.verified ? 'Edit Email ID' : 'Add Email ID'}</button></div></div>
