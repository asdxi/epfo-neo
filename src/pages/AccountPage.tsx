...
export interface AccountPageProps {
  ...
  onChangePassword?: (changedOn: string) => void
...
export function AccountPage(props: AccountPageProps) {
  ...
  return <section className="service-page account-page" aria-labelledby="account-title">
    <header className="service-page-heading"><h1 id="account-title">Profile and account</h1><p>Review your profile, security settings and account preferences.</p></header>
    <AccountSection title="Profile" description="Your profile information is used to match your EPFO member record."><ProfileOverview account={account} onUpdateProfile={props.onUpdateProfile} /></AccountSection>
    <AccountSection title="Security" description="Sensitive changes require Aadhaar OTP verification."><ContactEditor {...props} /><PasswordEditor onChangePassword={props.onChangePassword} /></AccountSection>
...
function ProfileOverview({ account, onUpdateProfile }: Pick<AccountPageProps, 'account' | 'onUpdateProfile'>) {
  ...
  const changePhoto = (file?: File) => {
    ...
    if (file.size > 2 * 1024 * 1024) { setPhotoError('Choose a photograph smaller than 2 MB for this demo.'); return }
  ...
  return <div className="profile-overview"><div className="profile-photo-panel"><img src={member.passportPhotoUrl} alt="Member passport photograph" /><div><strong>Passport photograph</strong><p>Use a digital-camera photo in JPEG, JPG or PNG format. Keep the face clearly visible with both ears visible. Use the 3.5 cm × 4.5 cm portrait ratio.</p><label className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md profile-photo-upload">Change photograph<input type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" onChange={(event) => changePhoto(event.target.files?.[0])} /></label>{photoError && <p className="service-field-error" role="alert">{photoError}</p>}{photoStatus && <p className="account-saved-message" role="status">{photoStatus}</p>}</div></div><dl className="account-detail-list profile-detail-list"><div><dt>Full name</dt><dd>{member.name}</dd></div><div><dt>UAN</dt><dd>{member.uan.replace(/(\d{4})(?=\d)/g, '$1 ')}</dd></div><div><dt>Date of birth</dt><dd>{formatDate(member.dateOfBirth)}</dd></div><div><dt>Father’s/Husband’s name</dt><dd>{member.fatherOrHusbandName}</dd></div><div><dt>Relationship</dt><dd>{member.relationship === 'father' ? 'Father' : 'Husband'}</dd></div><div><dt>International worker</dt><dd>{member.internationalWorker ? 'Yes' : 'No'} <small className="profile-locked">Locked after UAN activation</small></dd></div><div><dt>Educational qualification</dt><dd>{qualificationLabel[member.educationalQualification]}</dd></div><div><dt>Marital status</dt><dd>{member.maritalStatus === 'widow-widower' ? 'Widow/Widower' : titleCase(member.maritalStatus)}</dd></div><div><dt>Permanent address</dt><dd>{member.permanentAddress}</dd></div><div><dt>Current address</dt><dd>{member.currentAddress}</dd></div><div><dt>Differently abled</dt><dd>{member.differentlyAbled ? 'Yes' : 'No'}</dd></div><div><dt>Profile last updated</dt><dd>{formatDate(member.profileUpdatedOn)}</dd></div><div><dt>Last password change</dt><dd>{formatDate(member.passwordChangedOn)}</dd></div></dl></div>
}

function PasswordEditor({ onChangePassword }: Pick<AccountPageProps, 'onChangePassword'>) {
  const [step, setStep] = useState<'read' | 'edit' | 'otp' | 'review' | 'saved'>('read')
  const [password, setPassword] = useState('')
  const [otp, setOtp] = useState('')
  const [error, setError] = useState('')
  if (step === 'read') return <div className="account-inline-state"><div><strong>Password</strong><span>Use a strong password you do not reuse elsewhere.</span></div><button className="ux4g-btn ux4g-btn-tonal-primary ux4g-btn-md" type="button" onClick={() => setStep('edit')}>Change password</button></div>
  if (step === 'saved') return <div className="ux4g-alert ux4g-alert-success" role="status"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Password changed</p><p className="ux4g-alert-message">Your password change was confirmed using Aadhaar OTP.</p><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('read')}>Done</button></div></div>
  if (step === 'otp') return <form className="contact-editor" onSubmit={(event) => { event.preventDefault(); if (otp !== DEMO_OTP) { setError('We could not verify this Aadhaar OTP. Check the code and try again.'); return }; setError(''); setStep('review') }}><div className="service-field"><label htmlFor="password-otp">Aadhaar OTP</label><input id="password-otp" className="ux4g-input ux4g-input-md" inputMode="numeric" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} />{error && <p className="service-field-error" role="alert">{error}</p>}</div><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('edit')}>Back</button><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="submit">Verify Aadhaar OTP</button></div></form>
  if (step === 'review') return <div className="contact-editor"><h3>Review password change</h3><p>Your password will be changed after Aadhaar OTP verification.</p><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('edit')}>Back</button><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="button" onClick={() => { onChangePassword?.(new Date().toISOString().slice(0, 10)); setStep('saved') }}>Change password</button></div></div>
  return <form className="contact-editor" onSubmit={(event) => { event.preventDefault(); if (password.length < 8) { setError('Use at least 8 characters.'); return }; setError(''); setStep('otp') }}><div className="service-field"><label htmlFor="new-password">New password</label><input id="new-password" className="ux4g-input ux4g-input-md" type="password" value={password} onChange={(event) => setPassword(event.target.value)} />{error && <p className="service-field-error" role="alert">{error}</p>}</div><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('read')}>Cancel</button><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="submit">Continue to Aadhaar OTP</button></div></form>
}
...
function ContactEditor({ account, onUpdateContact }: AccountPageProps) {
  ...
  if (step === 'otp') return <form className="contact-editor" data-otp-destination="aadhaar-linked-mobile" onSubmit={verifyOtp}><div className="ux4g-alert ux4g-alert-info contact-verification-alert"><div className="ux4g-alert-content"><p className="ux4g-alert-title">Aadhaar OTP verification required</p><p className="ux4g-alert-message">An Aadhaar OTP was sent to the Aadhaar-linked mobile number on your account.</p></div></div><div className="service-field"><label htmlFor="contact-otp">Aadhaar OTP</label><input id="contact-otp" className={`ux4g-input ux4g-input-md ${errors.otp ? 'ux4g-input-error' : ''}`} inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={(event) => { setOtp(event.target.value.replace(/\D/g, '').slice(0, 6)); setErrors({}) }} aria-invalid={Boolean(errors.otp)} aria-describedby={errors.otp ? 'contact-otp-error' : 'contact-otp-help'} />{errors.otp ? <p id="contact-otp-error" className="service-field-error" role="alert">{errors.otp}</p> : <p id="contact-otp-help" className="service-field-help">Enter the six-digit Aadhaar OTP to confirm this change.</p>}</div><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('edit')}>Back</button><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="submit">Verify Aadhaar OTP</button></div></form>
