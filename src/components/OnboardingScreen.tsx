...
type Step = 'activate' | 'profile' | 'verify' | 'face' | 'complete'

export interface OnboardingScreenProps {
  onComplete: (profile: Pick<Member, 'fatherOrHusbandName' | 'relationship' | 'internationalWorker' | 'educationalQualification' | 'maritalStatus' | 'permanentAddress' | 'currentAddress' | 'differentlyAbled'>, faceAuthenticationState: Member['faceAuthenticationState']) => void
  onBack: () => void
  demoMode?: 'happy' | 'error'
  onExitErrorState?: () => void
...
const onboardingSteps = ['UAN And Aadhaar Details', 'Aadhaar OTP', 'Face Authentication', 'Complete']

export function OnboardingScreen({ onComplete, onBack, demoMode = 'happy', onExitErrorState = () => undefined, onNotify = () => undefined }: OnboardingScreenProps) {
  ...
  const [faceAuthenticationState, setFaceAuthenticationState] = useState<Member['faceAuthenticationState']>('pending')
  const [faceError, setFaceError] = useState(false)
  const [faceAttempt, setFaceAttempt] = useState(0)
  const otpInputs = useRef<Array<HTMLInputElement | null>>([])
  const stepIndex = step === 'activate' || step === 'profile' ? 0 : step === 'verify' ? 1 : step === 'face' ? 2 : 3
  ...
  return <main className={`otp-login onboarding ${step === 'complete' ? 'is-complete' : ''}`} aria-labelledby="onboarding-title">
    <section className="otp-login__hero"><img className="brand-mark" src={logoEpfoNeo} alt="" aria-hidden="true" /><h1 id="onboarding-title">Activate your UAN</h1><p>Four short steps to set up access to your EPFO member account.</p></section>
    <section className="otp-login__card">
      ...
      {step === 'verify' && <form className="otp-login__form" onSubmit={verify} noValidate><div className="otp-login__step-heading"><h2>Verify with Aadhaar OTP</h2><p className="otp-login__supporting">Step 2 of 4. A six-digit code was sent to your Aadhaar-linked mobile number ending {mobile.slice(-4)}.</p></div><div className="ux4g-otp"><label className="ux4g-otp-label" id="register-otp-label">Aadhaar OTP <span className="field-required" aria-hidden="true">*</span></label><div className="ux4g-otp-group" aria-labelledby="register-otp-label">{otp.map((digit, index) => <input key={index} ref={(element) => { otpInputs.current[index] = element }} className="ux4g-input ux4g-input-md ux4g-otp-slot ux4g-otp-input" type="text" inputMode="numeric" autoComplete={index === 0 ? 'one-time-code' : 'off'} maxLength={1} value={digit} onChange={(event) => updateOtp(index, event.target.value)} onPaste={(event) => { event.preventDefault(); pasteOtp(index, event.clipboardData.getData('text')) }} onKeyDown={(event) => { if (event.key === 'Backspace' && !otp[index] && index > 0) otpInputs.current[index - 1]?.focus() }} aria-label={`Aadhaar OTP digit ${index + 1}`} />)}</div></div><OtpResend onResend={() => onNotify({ tone: 'success', title: 'OTP Sent', message: 'A new OTP was sent to your Aadhaar-linked mobile number.' })} /><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-text-primary ux4g-btn-md" type="button" onClick={() => setStep('profile')}>Back</button><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="submit" disabled={otpValue.length !== DEMO_OTP.length}>Verify Aadhaar OTP</button></div></form>}
      {step === 'face' && <section className="otp-login__form" aria-labelledby="face-authentication-title">{faceError ? <div className="demo-error-state"><span className="demo-error-state__icon" aria-hidden="true">!</span><div><h2 id="face-authentication-title">Face Authentication Couldn’t Be Completed</h2><p>Your details and Aadhaar OTP are saved for this attempt. UAN activation is not complete yet.</p><p>The verification service is temporarily unavailable. Try again or exit the simulation.</p></div><div className="service-flow-actions"><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="button" onClick={() => { setFaceError(false); setFaceAttempt((attempt) => attempt + 1) }}>Try Face Authentication Again</button><button className="ux4g-btn ux4g-btn-outline-danger ux4g-btn-md" type="button" onClick={onExitErrorState}>Exit Error State</button></div></div> : <><div className="otp-login__step-heading"><h2 id="face-authentication-title">Face Authentication</h2><p className="otp-login__supporting">Step 3 of 4. Complete this simulation to finish UAN activation.</p></div><FaceAuthenticationDemo key={faceAttempt} onComplete={() => { if (demoMode === 'error') { setFaceError(true); return } setFaceAuthenticationState('verified'); setStep('complete') }} onContinueForNow={() => { setFaceAuthenticationState('pending'); setStep('complete') }} /></>}</section>}
      {step === 'complete' && <section className="onboarding-success" aria-labelledby="activation-complete-title"><span className="onboarding-success__icon" aria-hidden="true">✓</span><div><h2 id="activation-complete-title">{faceAuthenticationState === 'verified' ? 'UAN Activation Complete' : 'Face Authentication Pending'}</h2><p>{faceAuthenticationState === 'verified' ? 'Your profile is ready. You can now view your EPF records.' : 'You can use a limited EPFO Neo experience and complete face authentication later from Profile.'}</p></div><button className="ux4g-btn ux4g-btn-primary ux4g-btn-md" type="button" onClick={() => onComplete(profile, faceAuthenticationState)}>View My Account</button></section>}
    </section>
  </main>
}

function OnboardingStepper({ activeIndex }: { activeIndex: number }) { return <div className="service-progress-wrap onboarding-stepper"><ol className="service-progress" aria-label={`Onboarding progress: step ${activeIndex + 1} of ${onboardingSteps.length}`}>{onboardingSteps.map((label, index) => <li key={label} className={index < activeIndex ? 'is-complete' : index === activeIndex ? 'is-current' : ''} aria-current={index === activeIndex ? 'step' : undefined}><span className="service-progress-step-number" aria-hidden="true">{index + 1}</span><span className="service-progress-label">{label}</span></li>)}</ol></div> }
