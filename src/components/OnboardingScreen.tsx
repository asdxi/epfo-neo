...
export function OnboardingScreen({ onComplete, onBack }: OnboardingScreenProps) {
  ...
  return <main className="otp-login onboarding" aria-labelledby="onboarding-title">
    ...
    <section className="otp-login__card">
      ...
      {step === 'face' && <section className="otp-login__form" aria-labelledby="face-authentication-title"><div className="otp-login__step-heading"><h2 id="face-authentication-title">Face Authentication</h2><p className="otp-login__supporting">Step 3 of 4. Complete this simulation to finish UAN activation.</p></div><FaceAuthenticationDemo onComplete={() => { setFaceAuthenticationState('verified'); setStep('complete') }} onContinueForNow={() => { setFaceAuthenticationState('pending'); setStep('complete') }} /></section>}
